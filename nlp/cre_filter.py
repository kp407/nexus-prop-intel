...
SPACE_VERBS = [
    ...
    "operationalise", "operationalize", "go live",
]
...
SPACE_NOUNS = [
    ...
    "data centre", "data center", "warehouse", "logistics park",
...
def is_cre_relevant(title: str, text: str) -> tuple[bool, float, str]:
    ...
    if cre_score == 0:
        # Last chance: check for strong intent phrases even without verb+noun combo
        strong_intents = [
            "gcc", "global capability centre", "global delivery centre",
            "captive centre", "co-location campus", "colocation campus",
        ...
        if any(phrase in combined for phrase in strong_intents):
            cre_score = 3  # treat as weak CRE signal, let intent layer handle it
        else:
